...
if(file_exists($datadir.'config/_db_config.php')) {
	exit("<br>ALEC BOARD가 이미 설치되어있습니다.<br><br>다시 설치하시려면 아래 파일을 지워주세요.<br>./data/config/_db_config.php");
}
...
if(empty($_POST['db_name'])) {
	...
	echo '<h3>ALEC BOARD V0.1 BETA 설치</h3><form action="index.php" method="post" autocomplete="off">';
	...
	echo '<span style="display:inline-block;padding-left:163px">쿠키 도메인 www.alexleejw.com 와 alexleejw.com 은 서로 다른 도메인으로 인식합니다.<br>쿠키를 사용 & 공유하시려면 .alexleejw.com 과 같이 입력하세요.</span><br><br>';
...
if (empty($row['th_id'])) {
	$tmp = [];
	$tmp['carousel_item_1'] = '<h1>헤드라인</h1><br><p>이것은 ALEC BOARD의 헤드라인 예제입니다.<br>이 헤드라인은 테마 설정에서 사용자가 원하는 대로 작성하시면 됩니다.<br>더 자세한 내용이 궁금하시면 프로젝트 페이지에 방문해주세요.</p><br><a class="btn btn-primary" href="#">오늘 가입</a>';
	$tmp['carousel_item_2'] = '<h1>두번째 헤드라인</h1><br><p>ALEC BOARD는 편하고 쉽게 콘텐츠를 발행을 할 수 있도록 하기 위한 CMS입니다.<br>확장형 구조로 모듈, 애드온, 위젯, 에디터, 테마를 기반으로 결과물을 생성합니다.</p><br><br><a class="btn btn-primary" href="#">자세히 알아보기</a>';
	$tmp['carousel_item_3'] = '<h1>마지막 헤드라인</h1><br><p>ALEC BOARD는 각각의 기능과 디자인이 구조적으로 연결되는 모듈형 구조로 개발 및 유지보수를 쉽게 하도록 도와주며 관리자는 손쉽게 설정과 디자인을 변경할 수 있으며 여러분만의 개성을 가진 웹 사이트를 만들 수 있습니다.</p><br><a class="btn btn-primary" href="#">갤러리 검색</a>';
	$tmp['footer_html'] = 'ALEC BOARD는 <a href="http://alexleejw.com" target="_blank">ALEXLEE.COM</a>에 의해 디자인되고 만들어 졌으며 <a href="https://github.com/Sjdnejd/ALECBOARD">코드 작성자</a>의 도움과 <a href="https://github.com/Sjdnejd">관리자</a>에 의해 업데이트 됩니다.';
	$tmp = "'".str_replace(['\\',"\0","\n","\r","'",'"',"\x1a"],['\\\\','\\0','\\n','\\r',"\\'",'\\"','\\Z'],serialize($tmp))."'";
...
if($error = DB::error()) throw new Exception($error->getMessage(),$error->getCode());
if (empty($row['theme'])) {
	$sql = 'INSERT INTO '._AF_CONFIG_TABLE_.' (`lang`,`theme`, `start`, `title`, `use_signup`) VALUES ("ko", "default", "welcome", "알렉보드", "1")';
	DB::query($sql);
}
...
if (empty($row['md_id'])) {
	...
	DB::query(sprintf($sql, 'welcome', 'page', 'ALECBOARD'));
}
